...
def randn(shape, dtype=None):
    ...
    x = uniform(shape)
    y = uniform(shape)
    normal_sample = ops.pow(-2 * ops.log(x), 0.5) * ops.cos(2 * Tensor(pi, dtype) * y)
...
def clip_grad_norm(grads, max_norm: float, norm_type: int = 2, error_if_nonfinite: bool = False):
